...
public class Model {
    ...
    public static boolean attackSelected(int r, int c) {
        TerrainTile enemyTile = map.getTile(r, c);
        MapObject enemyObject = enemyTile.getOccupant();
        if (enemyObject == null && !(enemyObject instanceof MilitaryUnit)) {
            return false;
        }
        MilitaryUnit enemyMilitaryUnit = (MilitaryUnit) enemyObject;

        MapObject myObject = selected.getOccupant();
        if (myObject == null && !(myObject instanceof MilitaryUnit)) {
            return false;
        }
        MilitaryUnit myMilitaryUnit = (MilitaryUnit) myObject;

        if (myMilitaryUnit.getCanAttack() && (enemyMilitaryUnit.getOwner()
            == myMilitaryUnit.getOwner())) {
            return false;
        }

        myMilitaryUnit.attack(enemyMilitaryUnit);
        if (enemyMilitaryUnit.isDestroyed()) {
